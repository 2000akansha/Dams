...
import Verification from "../webModel/verification.js";

// DONT TOUCH (shabaaz's Code)
...
export const getAllbeneficiaryDisburmentlist = async (req, res) => {
  try {
    ...
    // Group beneficiaries by khatauniId
    const groupedByKhatauni = transformedBeneficiaries.reduce((acc, b) => {
      const khatauniId = b.khatauniSankhya || "Unknown"; // Use khatauniSankhya or fallback if not available
      if (!acc[khatauniId]) {
        acc[khatauniId] = [];
      }
      acc[khatauniId].push(b); // Group beneficiaries under khatauniId
      return acc;
    }, {});

    // Filter based on user role
    const filteredGroupedByKhatauni = Object.entries(groupedByKhatauni).reduce(
      (acc, [khatauniId, beneficiaries]) => {
        ...
        if (filteredBeneficiaries.length > 0) {
          acc[khatauniId] = filteredBeneficiaries; // Only add khatauni if it has filtered beneficiaries
        }
        return acc;
      },
      {}
    ...
    // Return grouped and filtered beneficiaries by khatauniId
    return res.status(200).json({
      success: true,
      beneficiaries: filteredGroupedByKhatauni,
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
...
export const sendToInputer = catchAsyncError(async (req, res, next) => {
  try {
    if (!req.user) {
      return next(new ErrorHandler('User is not the authorised user.', 400))
    }
    const updateInputer = await Verification.findOneAndUpdate({ verificationLevel: '0' })
    if (!updateInputer) {
      return next(new ErrorHandler('Sending to Inputer failed.', 400))

    }
    return res.status(200).json({
      status: true,
      message: 'Beneficiary sent to Inputer.'
    });
  }
  catch (error) {
    next(error)
